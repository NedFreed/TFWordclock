...
// Modified by Ned Freed to support generation of days-since-Jan-1-1970
// tables of DST transitions for use in Arduino sketches and similar
// severely memory-constrained environments. This features is activated by
// the -o switch.
...
// range to [INT_MIN:INT_MAX] even on LP64.
	...
	vflag = 0;
	cutoff = NULL;
	while ((c = getopt(argc, argv, "c:vo")) == 'c' || c == 'v' || c == 'o')
		if (c == 'v')
			vflag = 1;
		else if (c == 'o') {
			time(&offsets);
			offsets /= 60*60*24*365;
			offsets *= 60*60*24*365;
		}
		else	cutoff = optarg;
	if ((c != -1) ||
...
{
	fprintf(stderr,
_("usage: zdump [--version] [-v] [-c cutoff] [-o] zonename ...\n"));
	exit(EXIT_FAILURE);
}
...
{
	struct tm *	tmp;
	static long int	lasto = -1, o;
	static int	lastisdst = 0;

	if (offsets) {
		if (t < offsets)
			return;
		tmp = localtime(&t);
		if (lastisdst == tmp->tm_isdst)
			return;
		lastisdst = tmp->tm_isdst;
		o = t / (60 * 60 * 24);
		if (o == lasto)
			return;
		if (lasto < 0)
			(void) printf("const uint16_t dstinfo[DSTLENGTH] PROGMEM = {\n");
		lasto = o;
		(void) printf("  %ld, // ", o);
	}
	else {
